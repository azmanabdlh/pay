...
class Pay::Webhook::Test < ActiveSupport::TestCase
  test "rehydrates a Paddle Classic event" do
    pay_webhook = Pay::Webhook.create processor: :paddle_classic, event_type: :example,
      event: json_fixture("paddle_classic/subscription_payment_succeeded")
  ...
  test "rehydrates a Paddle Billing event" do
    pay_webhook = Pay::Webhook.create processor: :paddle_billing, event_type: :example,
      event: json_fixture("paddle_billing/subscription.created")
    event = pay_webhook.rehydrated_event
    assert_equal "month", event.billing_cycle.interval
  end

  test "rehydrates a Stripe event" do
    pay_webhook = Pay::Webhook.create processor: :stripe, event_type: :example,
      event: json_fixture("stripe/customer.updated")
    event = pay_webhook.rehydrated_event
  ...
  test "rehydrates a Braintree event" do
    pay_webhook = Pay::Webhook.create processor: :braintree, event_type: :example,
      event: json_fixture("braintree/subscription_charged_successfully")
    event = pay_webhook.rehydrated_event
