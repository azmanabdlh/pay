...
class Pay::Stripe::Webhooks::SubscriptionUpdatedTest < ActiveSupport::TestCase
  ...
  test "subscription is updated" do
    subscription = @pay_customer.subscriptions.create!(processor_id: @event.data.object.id, name: "default",
  ...
  test "subscription is updated with cancel_at_period_end = true and on_trial? = false" do
    subscription = @pay_customer.subscriptions.create!(processor_id: @event.data.object.id, name: "default",
      processor_plan: "some-plan", status: "active")
    ::Stripe::Subscription.stubs(:retrieve).returns fake_stripe_subscription(cancel_at_period_end: true, ended_at: nil,
      cancel_at: nil)
    Pay::Stripe::Webhooks::SubscriptionUpdated.new.call(@event)
  ...
  test "subscription is updated with cancel_at_period_end = true and on_trial? = true" do
    ...
    data[:items][:data][0][:current_period_end] = trial_end.to_i
    ::Stripe::Subscription.stubs(:retrieve).returns ::Stripe::Subscription.construct_from(data)
    Pay::Stripe::Webhooks::SubscriptionUpdated.new.call(@event)
  ...
  test "ended subscription sets end to ended_at" do
    ...
    ::Stripe::Subscription.stubs(:retrieve).returns fake_stripe_subscription(cancel_at_period_end: false,
      ended_at: sub_end.to_i)
  ...
  test "subscription was canceled, now renewed" do
    subscription = @pay_customer.subscriptions.create!(processor_id: @event.data.object.id, name: "default",
      processor_plan: "some-plan", status: "active", ends_at: Time.now)
    ::Stripe::Subscription.stubs(:retrieve).returns fake_stripe_subscription(cancel_at_period_end: false,
